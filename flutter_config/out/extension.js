...
function updateMainDartForRouterAndScreenUtil(rootPath) {
...
async function addCleanArchApiAction(rootPath, featureName, featurePascal, actionCamel, actionPascal, httpMethod, endpoint, stateMgmt, packageName) {
...
async function addMvvmApiAction(rootPath, featureName, featurePascal, actionCamel, actionPascal, httpMethod, endpoint, stateMgmt, packageName) {
    const featDir = path.join(rootPath, 'lib', 'features', featureName);
    const vmDir = path.join(featDir, 'viewmodels');
    if (stateMgmt === 'BLoC') {
