...
function activate(context) {
    ...
    let createScreenDisposable = vscode.commands.registerCommand('flutter-config.createScreen', async (uri) => {
        const workspaceFolder = vscode.workspace.workspaceFolders?.[0];
        if (!workspaceFolder) {
            vscode.window.showErrorMessage('Please open a Flutter workspace first.');
            return;
        ...
        const config = vscode.workspace.getConfiguration('flutterConfig');
        let arch = config.get('architecture');
    ...
    let generateForExistingScreenDisposable = vscode.commands.registerCommand('flutter-config.generateForExistingScreen', async (uri) => {
        const workspaceFolder = vscode.workspace.workspaceFolders?.[0];
        if (!workspaceFolder) {
            vscode.window.showErrorMessage('Please open a Flutter workspace first.');
            return;
    ...
    let createCubitDisposable = vscode.commands.registerCommand('flutter-config.createCubit', async (uri) => {
        const workspaceFolder = vscode.workspace.workspaceFolders?.[0];
        if (!workspaceFolder) {
            vscode.window.showErrorMessage('Please open a Flutter workspace first.');
            return;
...
`;
    ...
    let generateApiActionDisposable = vscode.commands.registerCommand('flutter-config.generateApiAction', async () => {
        ...
        const featureCamel = toCamelCase(featureName);
        const config = vscode.workspace.getConfiguration('flutterConfig');
        let arch = config.get('architecture') || 'Clean Architecture (Feature-First)';
        let stateMgmt = config.get('stateManagement') || 'BLoC';
        ...
        const httpMethod = await vscode.window.showQuickPick(['GET', 'POST', 'PUT', 'DELETE'], { placeHolder: 'Select HTTP Method for this API call', ignoreFocusOut: true });
        if (!httpMethod)
            return;
    ...
    let generateSignedAppBundleDisposable = vscode.commands.registerCommand('flutter-config.generateSignedAppBundle', async () => {
        ...
        await vscode.window.withProgress({
            ...
            await new Promise((resolve) => {
                (0, child_process_1.exec)(buildCmd, { cwd: rootPath, env: keytoolResolution.env }, (err, stdout, stderr) => {
                    if (err) {
    ...
    let initCICDDisposable = vscode.commands.registerCommand('flutter-config.initCICD', async () => {
        ...
        const rootPath = workspaceFolder.uri.fsPath;
        const buildTarget = await vscode.window.showQuickPick(['APK', 'App Bundle (AAB)', 'Both'], { placeHolder: 'Select the Android build artifact for CI', ignoreFocusOut: true });
        if (!buildTarget)
            return;
        await vscode.window.withProgress({
            location: vscode.ProgressLocation.Notification,
...
class ScreenCodeActionProvider {
...
async function promptForModels(featurePascal, featureName, featureCamel) {
    const models = [];
    const modelMode = await vscode.window.showQuickPick(['No models (Empty template)', 'Single Model / API', 'Multiple Models / APIs'], { placeHolder: 'Does this feature need API data models?', ignoreFocusOut: true });
...
function parseJsonToModel(name, inputJson) {
...
function isInjectableEnabled(rootPath) {
    try {
        const pubspecPath = path.join(rootPath, 'pubspec.yaml');
        if (fs.existsSync(pubspecPath)) {
            const content = fs.readFileSync(pubspecPath, 'utf8');
...
function buildActionCallSnippet(stateMgmt, arch, featureCamel, featurePascal, actionCamel, actionPascal) {
...
exports.generateCoreConfig = generateCoreConfig;
function updateMainDartForRouterAndScreenUtil(rootPath, stateMgmt) {
...
exports.updateMainDartForRouterAndScreenUtil = updateMainDartForRouterAndScreenUtil;
function registerRouteInGoRouter(rootPath, name, pascal, packageName, arch, stateMgmt) {
...
exports.registerRouteInGoRouter = registerRouteInGoRouter;
function registerDependenciesInLocator(rootPath, name, pascal, packageName, arch, stateMgmt, models) {
    if (isInjectableEnabled(rootPath))
        return;
...
exports.registerDependenciesInLocator = registerDependenciesInLocator;
function generateCleanArchFiles(targetDir, name, pascal, camel, stateMgmt, models, packageName, skipPageIfExists = false) {
    ...
    for (const m of models) {
        ...
        fs.writeFileSync(path.join(domainDir, 'entities', `${m.modelNameSnake}_entity.dart`), `class ${m.modelNamePascal}Entity {
...
`);
        ...
        fs.writeFileSync(path.join(domainDir, 'repositories', `${m.modelNameSnake}_repository.dart`), `import '../entities/${m.modelNameSnake}_entity.dart';
...
`);
        ...
        fs.writeFileSync(path.join(domainDir, 'usecases', `get_${m.modelNameSnake}_usecase.dart`), `${injectableImport}import '../entities/${m.modelNameSnake}_entity.dart';
...
`);
        // 3b. Create Usecase
        fs.writeFileSync(path.join(domainDir, 'usecases', `create_${m.modelNameSnake}_usecase.dart`), `${injectableImport}import '../entities/${m.modelNameSnake}_entity.dart';
import '../repositories/${m.modelNameSnake}_repository.dart';
...
`);
        // 3c. Update Usecase
        fs.writeFileSync(path.join(domainDir, 'usecases', `update_${m.modelNameSnake}_usecase.dart`), `${injectableImport}import '../entities/${m.modelNameSnake}_entity.dart';
import '../repositories/${m.modelNameSnake}_repository.dart';
...
`);
        // 3d. Delete Usecase
        fs.writeFileSync(path.join(domainDir, 'usecases', `delete_${m.modelNameSnake}_usecase.dart`), `${injectableImport}import '../repositories/${m.modelNameSnake}_repository.dart';
...
`);
        ...
        fs.writeFileSync(path.join(dataDir, 'models', `${m.modelNameSnake}_model.dart`), `import 'package:freezed_annotation/freezed_annotation.dart';
...
`);
        ...
        fs.writeFileSync(path.join(dataDir, 'datasources', `${m.modelNameSnake}_remote_data_source.dart`), `${dsInjectableImport}import 'package:dio/dio.dart';
...
`);
        ...
        fs.writeFileSync(path.join(dataDir, 'repositories', `${m.modelNameSnake}_repository_impl.dart`), `${repoInjectableImport}import '../../domain/entities/${m.modelNameSnake}_entity.dart';
...
`);
    ...
    if (stateMgmt === 'BLoC') {
        ...
        if (!skipPageIfExists || !fs.existsSync(blocPath)) {
            // Event
            fs.writeFileSync(eventPath, `part of '${name}_bloc.dart';
...
`);
            ...
            fs.writeFileSync(statePath, `part of '${name}_bloc.dart';
  ...
  final Delete${m.modelNamePascal}UseCase delete${m.modelNamePascal}UseCase;`).join('\n');
    ...
    required this.delete${m.modelNamePascal}UseCase,`).join('\n');
          ...
          ${models.map(m => `get${m.modelNamePascal}UseCase.execute().catchError((_) => null),`).join('\n          ')}
            ...
            fs.writeFileSync(blocPath, `import 'package:flutter_bloc/flutter_bloc.dart';
...
import '../../domain/usecases/get_${m.modelNameSnake}_usecase.dart';`).join('\n');
    ...
    return results;`;
            ...
            fs.writeFileSync(providerPath, `import 'package:riverpod_annotation/riverpod_annotation.dart';
import 'package:${packageName}/core/di/injection.dart';
${notifierImports}
...
import '../../domain/entities/${m.modelNameSnake}_entity.dart';`).join('\n');
            ...
            const controllerInjectableImport = useInjectable ? "import 'package:injectable/injectable.dart';\n" : "";
            const controllerInjectableAnnot = useInjectable ? "@injectable\n" : "";
            fs.writeFileSync(controllerPath, `import 'package:get/get.dart';
${controllerInjectableImport}${controllerImports}
...
exports.generateCleanArchFiles = generateCleanArchFiles;
function generateMVVMFiles(targetDir, name, pascal, camel, stateMgmt, models, packageName, skipPageIfExists = false) {
    const featDir = path.join(targetDir, name);
    ...
    for (const m of models) {
        ...
        fs.writeFileSync(path.join(modelsDir, `${m.modelNameSnake}_model.dart`), `import 'package:freezed_annotation/freezed_annotation.dart';
...
`);
    ...
    for (const m of models) {
        ...
        if (!fs.existsSync(servicePath)) {
            ...
            fs.writeFileSync(servicePath, `${serviceInjectableImport}import 'package:dio/dio.dart';
import '../models/${m.modelNameSnake}_model.dart';
...
`);
    ...
    if (stateMgmt === 'BLoC') {
        ...
        if (!skipPageIfExists || !fs.existsSync(vmPath)) {
            ...
            // ViewModel (as Cubit)
            fs.writeFileSync(vmPath, `import 'package:flutter_bloc/flutter_bloc.dart';
...
`);
        ...
        if (!skipPageIfExists || !fs.existsSync(vmPath)) {
            const vmInjectableImport = useInjectable ? "import 'package:injectable/injectable.dart';\n" : "";
            const vmInjectableAnnot = useInjectable ? "@injectable\n" : "";
            fs.writeFileSync(vmPath, `import 'package:get/get.dart';
${vmInjectableImport}${models.map(m => `import '../models/${m.modelNameSnake}_model.dart';\nimport '../services/${m.modelNameSnake}_api_service.dart';`).join('\n')}
...
exports.generateMVVMFiles = generateMVVMFiles;
async function addCleanArchApiAction(rootPath, featureName, featurePascal, actionCamel, actionPascal, httpMethod, endpoint, stateMgmt, packageName, responseModel = null) {
    ...
    if (responseModel) {
        ...
        fs.writeFileSync(modelPath, `import 'package:freezed_annotation/freezed_annotation.dart';
...
async function addMvvmApiAction(rootPath, featureName, featurePascal, actionCamel, actionPascal, httpMethod, endpoint, stateMgmt, packageName, responseModel = null) {
    const featDir = path.join(rootPath, 'lib', 'features', featureName);
    const vmDir = path.join(featDir, 'viewmodels');
    const modelsDir = path.join(featDir, 'models');
    const servicesDir = path.join(featDir, 'services');
      ...
      errorMessage.value = e.toString();
        ...
        if (fs.existsSync(vmPath)) {
            ...
            if (responseModel) {
                ...
                if (!content.includes(responseImport))
                    content = responseImport + content;
                if (!content.includes(serviceImport))
                    content = serviceImport + content;
...
async function addLocalStateAction(rootPath, featureName, featurePascal, actionCamel, actionPascal, stateMgmt, arch, packageName) {
    const featDir = path.join(rootPath, 'lib', 'features', featureName);
    const presDir = path.join(featDir, 'presentation');
    const vmDir = path.join(featDir, 'viewmodels');
    if (arch.includes('Clean')) {
...
function configurePubspecForEnvAndL10n(rootPath) {
    const pubspecPath = path.join(rootPath, 'pubspec.yaml');
    if (!fs.existsSync(pubspecPath))
        return;
...
function generateEnvFile(rootPath) {
...
function generateL10nConfig(rootPath) {
...
function updateMainDartForEnv(rootPath) {
    const mainPath = path.join(rootPath, 'lib', 'main.dart');
    if (!fs.existsSync(mainPath))
        return;
...
function updateMainDartForLocalization(rootPath) {
    const mainPath = path.join(rootPath, 'lib', 'main.dart');
    if (!fs.existsSync(mainPath))
        return;
    let content = fs.readFileSync(mainPath, 'utf8');
...
function configureAndroidManifestForNotifications(rootPath) {
...
function configureInfoPlistForNotifications(rootPath) {
...
function generateNotificationService(rootPath) {
...
function updateMainDartForNotifications(rootPath) {
    const mainPath = path.join(rootPath, 'lib', 'main.dart');
    if (!fs.existsSync(mainPath))
        return;
    let content = fs.readFileSync(mainPath, 'utf8');
...
function generateConnectivityFiles(rootPath) {
    const serviceDir = path.join(rootPath, 'lib', 'core', 'services');
    fs.mkdirSync(serviceDir, { recursive: true });
...
function generateSecurityFiles(rootPath) {
    const serviceDir = path.join(rootPath, 'lib', 'core', 'services');
    fs.mkdirSync(serviceDir, { recursive: true });
...
function generateThemeFiles(rootPath) {
...
function updateMainDartForTheme(rootPath) {
    const mainPath = path.join(rootPath, 'lib', 'main.dart');
    if (!fs.existsSync(mainPath))
        return;
    let content = fs.readFileSync(mainPath, 'utf8');
...
function updateDioForSslPinning(rootPath) {
...
function updateDioForConnectivity(rootPath) {
    const dioPath = path.join(rootPath, 'lib', 'core', 'network', 'dio_configuration.dart');
    if (!fs.existsSync(dioPath))
        return;
    let content = fs.readFileSync(dioPath, 'utf8');
...
function configureAndroidSigning(rootPath, keystorePass) {
...
function generateGitHubActionsWorkflow(rootPath, buildTarget) {
...
function generateFlavorFiles(rootPath) {
...
exports.generateFlavorFiles = generateFlavorFiles;
function configureAndroidFlavors(rootPath) {
    const ktsPath = path.join(rootPath, 'android', 'app', 'build.gradle.kts');
    if (!fs.existsSync(ktsPath))
        return;
    let content = fs.readFileSync(ktsPath, 'utf8');
...
exports.configureAndroidFlavors = configureAndroidFlavors;
function configureIosFlavors(rootPath) {
...
function generatePaginatedListFiles(targetDir, name, pascal, itemModel, endpoint, packageName) {
    ...
    if (!fs.existsSync(modelPath)) {
        ...
        fs.writeFileSync(modelPath, `class ${itemPascal}Model {
${propertiesDecl}
...
`);
    ...
    fs.writeFileSync(controllerPath, `import 'package:flutter/foundation.dart';
import 'package:dio/dio.dart';
